...
function curtainPoints(width, height, data, time, layer, features) {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const y = baseY
      + (broad + secondary) * amplitude
      - spectral * amplitude * (.72 + layer * .1)
      - transient * amplitude;
...
export function drawAuroraFieldMode(context, width, height, data, accent, accent2, time, features = {}) {
  ...
  for (let layer = layers - 1; layer >= 0; layer -= 1) {
    ...
    fill.addColorStop(.52, layer % 2 === 0
      ? rgba(accent, .16 + bass * .055)
      : rgba(accent2 || accent, .13 + mid * .045, '205,89,255'));
