(() => {
  if (window.__shinobiNavigationStabilityV39) return;
  window.__shinobiNavigationStabilityV39 = true;
  ...
  function decorateNowPlayingBadges(root = document) {
    ...
    wraps.forEach(wrap => {
      wrap.querySelectorAll('span,small,[class*="badge"],[class*="context"]').forEach(element => {
        ...
        element.classList.add('now-playing-card-badge-v39');
      });
    });
  }
  ...
  // Deep/shared track routes must remain owned by Track Detail. Marking the
  // route before app-main starts prevents a brief Home render before detail.
  markPassiveTrackRoute();

  window.addEventListener('hashchange', event => {
    // Library Memory dispatches an untrusted synthetic HashChangeEvent while
    // restoring the last audio position; do not turn that into a detail route.
