...
The builder copies the runtime verbatim. A Pages deployment must never contain a host-only patch missing from `main`. Build 57 keeps `motion-spring.js`, `pulse-reactor.js`, `bass-fracture.js`, `gravity-lens.js` and `bio-structure.js` in the canonical static shell, replaces Build 56 pose compression with integrated signal-driven kinetic flow, and advances the cache namespace to `shinobi-launchpad-v57`.
